<#
    .SYNOPSIS
        This Azure Automation runbook automates the scheduled stopping of specific virtual machines in an Azure subscription. 
    ...
    .DESCRIPTION
        ...
        each, ensuring that the VMs are in the proper state before stopping them if necessary.
...
workflow Stop-AzureVMs 
{
	...
	(
		...
		# Add variable for Azure VMs to be Stopped by this script
		[parameter(Mandatory=$false)]
	...
	# Retrieve machines to Start variable asset if not specified (these machines will be STOPPED)
	...
	inlineScript
	{
		...
		# Create the array of machine names which are listed to be stopped
		$Machines = $MachinesToStart.Split(",")
			
		# Process each VM from list of those to be stopped
		ForEach ($Machine in $Machines)
		{
			...
			{
				# Get the full VM object and stop the VM
				Write-Output "Stopping VM: [$($VM.Name)]."
				Get-AzureVM -ServiceName $VM.ServiceName -Name $VM.Name | Stop-AzureVM -force
			}
		}
